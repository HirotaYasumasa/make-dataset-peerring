...
def main():
  ...
  #   count += 1
  #   if len(split_data) != post_columns:
  #     print("==========Error==========")
  #     print(count)
  ...
  dataframes = []
  post_column_names = ['id', 'user_id', 'type', 'content', 'category', 'commented_at', 'audience', 'is_pinned', 'device_info', 'created_at', 'updated_at']

  for text_data in texts:
    split_text = custom_split(text_data)
    df = pd.DataFrame([split_text], columns=post_column_names)
